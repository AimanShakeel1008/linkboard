...
| Phase status | In progress — lesson (HTML) + 4 playground demos written; awaiting user's demo runs |
| Last updated | 2026-07-17 |
| Last session ended | Mid-phase checkpoint 2026-07-17 (lesson-format switch complete) |
| Next concrete action | User reads `lesson.html` and runs the 4 demos (disk_race, spawn_family, thread_garble, blocking_waiter) per its "Run it & watch the logs" section, then reports back |
...
- **Pending user action:** the checkpoint commit includes `git rm` of the two superseded `lesson.md` files — after it runs, `lesson.html` is the only lesson format left in the repo.
- **Design state (2026-07-17):** HTML lessons confirmed rendering correctly by user. `lesson.css` uses LITERAL colors only (no CSS custom properties — user's preview renderer silently dropped `var()`; keep it that way) with solid-color fallbacks before every gradient, full dark mode via explicit overrides, and pure-CSS animations (entrance rise, drifting banner gradient, hover lifts, details reveal) guarded by a `prefers-reduced-motion` kill-switch.
...
- **Exact next step on resume:** Confirm the checkpoint push succeeded (it deletes the old lesson.md files). Then ask whether the four Phase 01 demos were run; interpret pasted output; then wrap the phase (end-of-phase workflow).
...
| Date | Summary |
|---|---|
| 2026-07-17 | **Lessons switched to styled HTML (user decision).** Built `concepts/assets/lesson.css` (shared design: gradient banner, orange/teal/violet palette, terminal-style code blocks, sticky TOC, light+dark) and converted Phase 00 + Phase 01 lessons to `lesson.html`. First render failed in user's preview (CSS custom properties dropped) → stylesheet rewritten with literal colors; user confirmed it renders beautifully. Added pure-CSS animations (entrance, gradient drift, hover lifts, details reveal, reduced-motion guard). HTML is now the only lesson format (CLAUDE.md "LESSON FORMAT" rule). Gave user a reusable prompt to replicate this setup in other projects. Session ended with a mid-phase checkpoint; Phase 01 demos still awaiting user run. |
